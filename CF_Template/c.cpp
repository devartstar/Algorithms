...
void solve() {
    ...
    sort(all(weight1), greater<int>());
    sort(all(weight2), greater<int>());
    sort(all(weight3), greater<int>());

    vector<str> dp(m+1);
    f1(curr_wt,m){
        if(curr_wt-1>=0){
            if(dp[curr_wt-1].a+1<(int)weight1.size() && dp[curr_wt-1].maxcost + weight1[dp[curr_wt-1].a + 1] > dp[curr_wt].maxcost){
        ...
        if(curr_wt-2>=0){
            if(dp[curr_wt-2].b+1<(int)weight2.size() && dp[curr_wt-2].maxcost + weight2[dp[curr_wt-2].b + 1] > dp[curr_wt].maxcost){
        ...
        if(curr_wt-3>=0){
            if(dp[curr_wt-3].c+1<(int)weight3.size() && dp[curr_wt-3].maxcost + weight3[dp[curr_wt-3].c + 1] > dp[curr_wt].maxcost){
