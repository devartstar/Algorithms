...
int query(int si, int segstart, int segend, int querystart, int queryend, int val){
    ...
    if(segstart >= querystart && segend<=queryend){  
        ll c = (lower_bound(all(segtree[si]), val) - segtree[si].begin());
        return c;
    }
...
void solve() {
    ...
    for(int i=1; i<=n; i++){
        m_first[v[i]]++;
        v_first[i] = m_first[v[i]];
    }

    buildMergeTree(1,1,n);
    ll ans = 0;
    f1(i,n){
        ans = ans + query(1,1,n,i+1,n,v_first[i]);
    }
    cout<<ans<<endl;
}
