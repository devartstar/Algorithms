...
void sieve()
{
    ...
    f(i,1,1000000)if(isprime[i])primes.pb(i);
}

void solve() {
	ll t,n,i,j,ans,temp,sum,sq;
	ans=temp=sum=0;
	cin>>n;
	sq = sqrt(n);
	sq++;
	auto it1 = lower_bound(all(primes),n);
	if(*it1 != n)it1--;
	auto it2 = lower_bound(all(primes),sq);
	ans = it1-it2+1;
	if(n == 1)ans=0;
	cout<<ans+1<<"\n";	
}
