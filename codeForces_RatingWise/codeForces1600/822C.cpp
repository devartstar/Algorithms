...
void solve() {
	...
	sort(all(v));
	vll bestCost(MAX_N, INF);
	ll ans = INF;
	f0(i, sz(v)) {
		if(v[i].ff.ss == -1) {
			ll duration = v[i].ss.ff - v[i].ff.ff + 1;
			if(duration <= x)
				ans = min(ans, v[i].ss.ss + bestCost[x - duration]);
		} else {
			ll duration = v[i].ff.ff - v[i].ss.ff + 1;
			bestCost[duration] = min(bestCost[duration], v[i].ss.ss);
		}
	}
	if(ans == INF)
		cout << -1 << endl;
