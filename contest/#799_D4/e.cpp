...
void solve() {
	int n, s;
	cin >> n >> s;
	vi v(n), freq(n, 0), rfreq(n, 0);
	f0(i, n) cin >> v[i];
	int ans = INF;
	f0(i, n) {
		if(i==0) freq[0] = v[0];
		else freq[i] = freq[i - 1] + v[i];
		if(freq[i] == s)
			ans = min(ans, i + 1);
	}
	if(freq[n-1] == s) {
	...
	debug(freq);
	for (int i = n - 1; i >= 0; i--) {
		if(i == n-1) rfreq[n-1] = v[n-1];
		else rfreq[i] = rfreq[i + 1] + v[i];
		if(v[i] == 1){
			ll left = s - rfreq[i];
			int pos = lower_bound(all(freq), left) - freq.begin();
			debug(mp(i, rfreq));
			debug(mp(left, pos));
			if(pos < n && freq[pos] == left) {
