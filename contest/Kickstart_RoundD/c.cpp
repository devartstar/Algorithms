...
void solve() {
	...
	f0r(i, n) {
		posta[i] = a[i];
		if(i < n-1) posta[i] += posta[i+1];
	}
	cin >> m;
	vll b(m), preb(m), postb(m);
	f0(i, m) {
		cin >> b[i];
		preb[i] = b[i];
		if(i > 0)
	...
	for (ll i = 0; i <= k; i++) {
		...
		for (ll j = 0; j <= i; j++) {
			// j from start
			ll fromStart = j, valStart = 0;
			if(fromStart > 0)
				valStart = prea[fromStart - 1];
			ll fromEnd = i-j, valEnd = 0;
			if(fromEnd > 0) {
