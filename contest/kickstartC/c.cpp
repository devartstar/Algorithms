...
void solve() {
	...
	f0(i, n) {
		...
		m[v[i]] = i+1;
		if(v[i].ss == 0) {
			out_time[i] = mp(v[i].ff, v[i].ss);
		} else if(v[i].ss == 1) {
			out_time[i] = mp(l - v[i].ff, v[i].ss);
		}
	}
	sort(all(v));
	sort(all(out_time));
	...
	f0(i, n) {
		if(out_time[i].ss == 0) {
			// left falls off
			ans.pb(mp(out_time[i].ff, m[v[left]]));
			left++;
		} else if(out_time[i].ss == 1) {
			// right falls off
			ans.pb(mp(out_time[i].ff, m[v[right]]));
			right--;
		}
	}
	sort(all(ans));
	f0(i, n) cout << ans[i].ss << " ";
	cout << endl;
}
